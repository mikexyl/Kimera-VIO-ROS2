...
def _create_mesh_splat_process(context, *_args, **_kwargs):
    workspace_root = LaunchConfiguration("workspace_root").perform(context)
    mesh_topic = LaunchConfiguration("mesh_topic").perform(context)
    texture_topic = LaunchConfiguration("texture_topic").perform(context)
    entity_path = LaunchConfiguration("entity_path").perform(context)
    rerun_url = LaunchConfiguration("rerun_url").perform(context)
    application_id = LaunchConfiguration("application_id").perform(context)
    wireframe_radius = LaunchConfiguration("wireframe_radius").perform(context)

    src_root = os.path.join(workspace_root, "src")
    pixi_manifest = os.path.join(src_root, "pixi.toml")
    mesh_splat_script = os.path.join(
        src_root,
        "Kimera-VIO-ROS2",
        "mesh_splat",
        "mesh_splat",
    ...
    command = (
        "mkdir -p /tmp/mesh_splat_ros_home && "
        f"cd {shlex.quote(src_root)} && "
        "XDG_CACHE_HOME=$PWD/../.cache "
        "ROS_HOME=/tmp/mesh_splat_ros_home "
        f"pixi run --manifest-path {shlex.quote(pixi_manifest)} "
        f"python {shlex.quote(mesh_splat_script)} "
        "--ros-args "
        f"-p mesh_topic:={shlex.quote(mesh_topic)} "
        f"-p texture_topic:={shlex.quote(texture_topic)} "
        f"-p entity_path:={shlex.quote(entity_path)} "
        f"-p rerun_url:={shlex.quote(rerun_url)} "
        f"-p application_id:={shlex.quote(application_id)} "
        f"-p wireframe_radius:={shlex.quote(wireframe_radius)}"
    )
...
def generate_launch_description():
    ...
    workspace_root_arg = DeclareLaunchArgument(
        ...
        description="Workspace root that contains src/pixi.toml.",
    )
    mesh_topic_arg = DeclareLaunchArgument(
        "mesh_topic",
        default_value="/kimera_vio_ros/mesh",
        description="PolygonMesh topic consumed by mesh_splat.",
    )
    texture_topic_arg = DeclareLaunchArgument(
        "texture_topic",
        default_value="/kimera_vio_ros/debug_mesh_img",
        description="Texture image topic consumed by mesh_splat.",
    )
    entity_path_arg = DeclareLaunchArgument(
        "entity_path",
        default_value="map/mesh_splat/mesh",
        description="Root entity path used by mesh_splat in Rerun.",
    )
    rerun_url_arg = DeclareLaunchArgument(
        "rerun_url",
        default_value="rerun+http://127.0.0.1:9876/proxy",
        description="Rerun gRPC endpoint.",
    )
    application_id_arg = DeclareLaunchArgument(
        "application_id",
        default_value="mesh_splat",
        description="Rerun application id used by mesh_splat.",
    )
    wireframe_radius_arg = DeclareLaunchArgument(
        "wireframe_radius",
        default_value="0.002",
        description="Wireframe line radius in meters.",
    )
    ...
    return LaunchDescription(
        [
            ...
            rerun_url_arg,
            application_id_arg,
            wireframe_radius_arg,
            play_bag_arg,
            bag_path_arg,
            bag_play_delay_arg,
            bag_clock_hz_arg,
            kimera_launch,
            OpaqueFunction(function=_create_mesh_splat_process),
            bag_play,
        ]
    )
