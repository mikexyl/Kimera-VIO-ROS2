...
StereoInterface::StereoInterface(rclcpp::Node::SharedPtr &node)
  ...
  image_sync_ = std::make_shared<ImageSync>(
      ImagePolicy(queue_size_), *left_image_sub_, *right_image_sub_);
  image_sync_->registerCallback(&StereoInterface::stereo_image_cb, this);
...
void StereoInterface::stereo_info_cb(
  ...
  msgCamInfoToCameraParams(left_msg, &vio_params_->camera_params_.at(0));
  msgCamInfoToCameraParams(right_msg, &vio_params_->camera_params_.at(1));
...
void StereoInterface::stereo_image_cb(const Image::SharedPtr left_msg,
